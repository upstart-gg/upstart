		...
		<div className="field field-string basis-full">
			...
			{schema["ui:multiline"] ? (
				<TextArea
					...
					className={tx(
						"!mt-1.5 scrollbar-thin",
						schema["ui:textarea-class"] ?? "h-24",
					)}
					required={required}
...
export const PathField: FC<FieldProps<string>> = (props) => {
	const { currentValue, onChange, required, title, description, placeholder } =
		props;
	const onChangeDebounced = useDebounceCallback(onChange, 300);
...
export const UrlOrPageIdField: FC<FieldProps<UrlOrPageIdSettings | null>> = (
	props,
) => {
	const {
		currentValue,
		...
		<div className="flex-1">
			<div className="flex justify-between flex-1 gap-1">
				...
				>
					...
					<SegmentedControl.Item value="pageId">
						Internal page
					</SegmentedControl.Item>
				</SegmentedControl.Root>
			</div>
...
export const GeoAddressField: FC<FieldProps<string>> = (props) => {
	...
	const onAddressChangeDebounced = useDebounceCallback(
