...
export default function PanelBrickInspector({ brick }: { brick: Brick }) {
	...
	const [tabsMapping, setTabsMapping] = useLocalStorage<
	...
	const showTabsList =
		!!manifest.props.properties.preset || !!manifest.props.properties.variants;
		...
		<div className={tx("flex flex-col h-full")}>
			<div className="basis-1/2 grow-0">
				<Callout.Root size="1" className="m-2">
					<Callout.Text size="1">
						<span className="font-semibold">Style presets</span> are
						pre-configured color styles that can be applied to your bricks to
				...
				<div className="grid grid-cols-3 gap-2 auto-rows-[3rem] flex-1 mx-2">
                   ...
                   rounded-md hover:opacity-80`,
								brick.props.preset === preset.const &&
									"outline outline-2 outline-upstart-400",
							)}
						>
							{preset.title}
						</button>
...
function VariantsTab({ brick, section }: { brick: Brick; section: Section }) {
	...
	const variantsNames: Record<string, string> =
		schema["ui:variant-names"] ?? {};

	if (Object.keys(variantsNames).length === 0) {
		console.warn(
			"No variants defined for brick %s in section %s",
			brick.type,
			section.id,
		);
		return null;
	...
	console.log(
		"VariantsTab for brick %s in section %s",
		brick.type,
		section.id,
		schema,
	);
		...
		<div className={tx("flex flex-col h-full")}>
			<div className="basis-1/2 grow-0">
				{shouldDisplay && (
					<Callout.Root size="1" className="m-2">
						<Callout.Text size="1">
							<span className="font-semibold">Variants</span> change the layout
							and/or behavior of the brick
						</Callout.Text>
					</Callout.Root>
				)}
					...
					{Object.entries(variantsNames).map(([variantKey, variantName]) => (
						<div key={variantKey} className="py-2.5">
							<VariantSelector
								label={variantName}
								// biome-ignore lint/suspicious/noExplicitAny: <explanation>
								values={schema.items.anyOf.filter(
									(vr: any) => vr["ui:variant-type"] === variantKey,
								)}
								currentVariants={brick.props.variants}
								onChange={(variants) => {
									console.debug(
										"setting variant %s to %o",
										variantKey,
										variants,
									);
									updateBrickProps(
		...
		return (
			<div className="flex items-center justify-between pr-2">
				...
				<Select.Root
					...
					onValueChange={(value) => {
						onChange(
							currentVariants.filter((v) => v !== currentValue).concat(value),
						);
					}}
				>
					<Select.Trigger
	...
	return (
		<form className={tx("flex flex-col justify-between h-full")}>
			{previewMode === "mobile" && (
				<Callout.Root size="1" className="m-2">
					<Callout.Text size="1">
						<strong>Note</strong>: You are editing mobile-only styles. Any
						changes here will only affect how the brick appears on mobile
						devices.
					</Callout.Text>
				</Callout.Root>
			)}
...
function ContentTab({
	brick,
	manifest,
}: { brick: Brick; manifest: BrickManifest }) {
	const { updateBrickProps } = useDraftHelpers();
	const previewMode = usePreviewMode();
