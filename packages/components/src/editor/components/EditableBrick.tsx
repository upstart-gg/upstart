...
const BrickContextMenu = forwardRef<HTMLDivElement, BrickContextMenuProps>(
	({ brick, isContainerChild, children }, ref) => {
		...
		const manifest = useBrickManifest(brick.type);
		const canMoveLeft = isContainerChild
			? draftHelpers.canMoveToWithinParent(brick.id, "left")
			: null;
		const canMoveRight = isContainerChild
			? draftHelpers.canMoveToWithinParent(brick.id, "right")
			: null;
						...
						<ContextMenu.Item
							...
							onClick={(e) => {
								navigator.clipboard
									...
									.then(() => {
										toast(
											"Brick copied to clipboard. You can paste it to another page.",
											{
												duration: 4000,
											},
										);
						...
						<ContextMenu.Sub>
							...
							<ContextMenu.SubContent>
								<ContextMenu.CheckboxItem
									...
									onCheckedChange={() =>
										draftHelpers.toggleBrickVisibilityPerBreakpoint(
											brick.id,
											"mobile",
										)
									}
										...
										<ContextMenu.Item
											onClick={(e) => {
												...
												navigator.clipboard.writeText(
													JSON.stringify(parentContainer),
												);
											}}
										>
											Copy container
