...
export function useResizable(
	...
	const getActiveDirection = useCallback(
		(handle: HTMLElement | null): string => {
			if (!handle?.classList.contains("resizable-handle")) return "";

			const directionClass = Array.from(handle.classList).find((cls) =>
				cls.startsWith("resizable-handle-"),
			);

			return directionClass
				? directionClass.replace("resizable-handle-", "")
				: "";
		},
		[],
	);
		...
		elements.forEach((element) => {
			...
			const interactable = interact(htmlElement).resizable({
				edges: {
					...
					right:
						".resizable-handle-e, .resizable-handle-ne, .resizable-handle-se",
					bottom:
						".resizable-handle-s, .resizable-handle-se, .resizable-handle-sw",
				...
				listeners: {
					start(event) {
						...
						const activeHandle = event.interaction.downEvent
				...
				const checkNodes = (nodes: NodeList) =>
					Array.from(nodes).some(
						(node) =>
							...
							((node as Element).matches?.(cssQuery) ||
								(node as Element).querySelector?.(cssQuery)),
					);

				return (
					checkNodes(mutation.addedNodes) || checkNodes(mutation.removedNodes)
				);
			});
