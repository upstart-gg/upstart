...
import type {
	CallContextProps,
	GenerationState,
} from "@upstart.gg/sdk/shared/context";
import { generateId } from "@upstart.gg/sdk/shared/bricks";
import type {
	GenericPageConfig,
	GenericPageContext,
} from "@upstart.gg/sdk/shared/page";
...
export interface EditorStateProps {
	...
	onDraftChange?: (
		draft: DraftState,
		siteAndPages: SiteAndPagesConfig,
	) => Promise<void>;
}
...
export interface EditorState extends EditorStateProps {
	...
	setImagesSearchResults: (
		images: EditorStateProps["imagesSearchResults"],
	) => void;
				...
				temporal(
					immer((set, _get) => ({
						...
						toggleTextEditMode: () =>
							set((state) => {
								state.textEditMode =
									!state.textEditMode || state.textEditMode === "default"
										? "large"
										: "default";
							}),
						...
						togglePanel: (panel) =>
							set((state) => {
								state.panel =
									panel && state.panel === panel ? undefined : panel;
							}),
						...
						deselectBrick: (brickId) =>
							set((state) => {
								if (
									...
									(!brickId || state.selectedBrickId === brickId)
								) {
									state.selectedBrickId = undefined;
								}
							}),
						...
						togglePanelPosition: () =>
							set((state) => {
								state.panelPosition =
									state.panelPosition === "left" ? "right" : "left";
							}),
					...
					partialize: (state) =>
						Object.fromEntries(
							Object.entries(state).filter(([key]) =>
								["chatVisible", "previewMode"].includes(key),
							),
						),
				},
			),
...
export interface DraftStateProps {
	...
	brickMap: Map<
		string,
		{ brick: Brick; sectionId: string; parentId: string | null }
	>;
...
export interface DraftState extends DraftStateProps {
	...
	addBrick: (
		brick: Brick,
		sectiondId: string,
		parentContainerId: Brick["id"] | null,
	) => void;
	...
	addDatasource: (
		datasource: ValueOfObj<NonNullable<Site["datasources"]>>,
	) => void;
	addDatarecord: (
		datarecord: ValueOfObj<NonNullable<Site["datarecords"]>>,
	) => void;

	setSitemap(sitemap: Site["sitemap"]): void;

	getPositionWithinParent: (brickId: Brick["id"]) => number | undefined;
	canMoveToWithinParent: (
		brickId: Brick["id"],
		to: "left" | "right",
	) => boolean;
				...
				immer((set, _get) => ({
						...
						set((state) => {
							...
							const newSection: Section = {
								...
								order: afterSectionId
									? state.sections.find((s) => s.id === afterSectionId)
					...
					pickTheme: (themeId) =>
						set((state) => {
							...
							if (!theme) {
								console.error(
									"Cannot pick theme %s, it does not exist",
									themeId,
								);
								return;
							}
					...
					isLastSection: (sectionId) => {
						const state = _get();
						return (
							state.sections.find((s) => s.order === state.sections.length - 1)
								?.id === sectionId
						);
					},
					...
					addDatasource: (datasource) =>
						set((state) => {
							...
							const existing = Object.values(state.datasources).find(
								(ds) => ds.id === datasource.id,
							);
							if (existing) {
								console.error(
									"Cannot add datasource %s, it already exists",
									datasource.id,
								);
								return;
							}
					...
					addDatarecord: (datarecord) =>
						set((state) => {
							...
							const existing = Object.values(state.datarecords).find(
								(dr) => dr.id === datarecord.id,
							);
							if (existing) {
								console.error(
									"Cannot add datarecord %s, it already exists",
									datarecord.id,
								);
								return;
							}
					...
					duplicateSection: (id) =>
						set((state) => {
							...
							if (!section) {
								console.error(
									"Cannot duplicate section %s, it does not exist",
									id,
								);
								return;
							}
							const newSection = {
								...
								bricks: section.bricks.map((brick) => ({
									...
									props: mergeIgnoringArrays({}, brick.props, {
										$children: (
											brick.props.$children as Brick[] | undefined
										)?.map((child) => ({
											...child,
											id: `b_${generateId()}`,
										})),
					...
					moveSectionUp: (sectionId) =>
						set((state) => {
							...
							// previous
							const previous = state.sections.find(
								(s) => s.order === section.order - 1,
							);
							const previousIndex = state.sections.findIndex(
								(s) => s.order === section.order - 1,
							);
							...
							if (parentId) {
								...
								if (parentBrick?.props.$children) {
									parentBrick.props.$children = (
										parentBrick.props.$children as Brick[]
									).filter((b) => b.id !== id);
								}
							} else if (section) {
					...
					duplicateBrick: (id) =>
						set((state) => {
							...
							if (!original) {
								console.error(
									"Cannot duplicate brick %s, it does not exist",
									id,
								);
								return;
							}
							...
							const deepCloneBrick = (brick: Brick): Brick => {
								...
								if (brick.props.$children) {
									...
									newBrick.props.$children = children.map((child) =>
										deepCloneBrick(child),
									);
								}
							...
							const updateBrickMap = (
								...
								if (brick.props.$children) {
									const children = brick.props.$children as Brick[];
									children.forEach((child) =>
										updateBrickMap(child, sectionId, brick.id),
									);
								}
							};
							...
							if (parentId) {
								...
								if (parentBrick?.props.$children) {
									...
									(parentBrick.props.$children as Brick[]).splice(
										originalIndex + 1,
										0,
										newBrick,
									);
								...
								const section = state.sections.find((s) => s.id === sectionId);
								if (section) {
									const originalIndex = section.bricks.findIndex(
										(b) => b.id === id,
									);
					...
					updateBrickProps: (id, props, isMobileProps) =>
						set((state) => {
							...
							if (brick) {
								if (isMobileProps) {
									brick.mobileProps = mergeIgnoringArrays(
										{},
										brick.mobileProps,
										props,
										{
					...
					updateSectionProps: (id, props, isMobileProps) =>
						set((state) => {
							...
							if (section) {
								if (isMobileProps) {
									section.mobileProps = mergeIgnoringArrays(
										{},
										section.mobileProps,
										props,
										{
									...
									// @ts-ignore
									section.props = mergeIgnoringArrays(
										{},
										section.props,
										props,
										{
					...
					moveBrickWithin: (id, to) =>
						set((state) => {
							...
							if (!parentBrick || !parentBrick.props.$children) {
								console.error(
									"Cannot move brick %s, it does not have a parent container",
									id,
								);
								return;
							}
							...
							if (currentIndex === -1) {
								console.error(
									"Cannot move brick %s, it is not found in its parent's children",
									id,
								);
								return;
							}
					...
					reorderBrickWithin: (brickId, fromIndex, toIndex) =>
						set((state) => {
							...
							if (!brickMapping) {
								console.error(
									"Cannot reorder brick %s, brick mapping not found",
									brickId,
								);
								return;
							}
					...
					moveBrickToContainerBrick: (id, parentId) =>
						set((state) => {
							...
							if (!brick || !parent || !brickMapping) {
								console.error(
									"Cannot move brick %s to new parent, brick or new parent is null",
									id,
								);
								return;
							}
							if (!("$children" in parent.props)) {
								console.error(
									"Cannot move brick %s, parent brick %s is not a container",
									id,
									parentId,
								);
								return;
							...
							if (currentParentId) {
								...
								if (currentParent?.props.$children) {
									...
									currentParent.props.$children = (
										currentParent.props.$children as Brick[]
					...
					moveBrickToSection: (id, sectionId, index) =>
						set((state) => {
							...
							const { sectionId: currentSectionId, parentId: currentParentId } =
								brickMapping;
							...
							if (currentParentId) {
								...
								if (currentSection) {
									currentSection.bricks = currentSection.bricks.filter(
										(b) => b.id !== id,
									);
								}
							}
					...
					getPositionWithinParent: (brickId) => {
						...
						if (parent && "$children" in parent.props) {
							return (parent.props.$children as Brick[]).findIndex(
								(b: Brick) => b.id === brickId,
							);
						}
					},
					...
					addBrick: (brick, sectionId, parentContainerId) =>
						set((state) => {
							...
							if (!section) {
								console.error(
									"Cannot add brick, section %s does not exist",
									sectionId,
								);
								return;
							}
							...
							if (parentContainerId) {
								console.log(
									"Adding brick to parent container",
									parentContainerId,
								);
								const parentBrick = getBrick(parentContainerId, state);
								if (!parentBrick) {
									console.warn(
										"Cannot add brick, parent container %s does not exist",
										parentContainerId,
									);
									return;
								}
								...
								if (!parentSection) {
									console.warn(
										"Cannot add brick, parent section %s does not exist",
										sectionId,
									);
									return;
								}
								// Find the parent brick from the section's bricks
								const parentBrickInSection = parentSection.bricks.find(
									(b) => b.id === parentContainerId,
								);
								...
								if (!parentBrickInSection.props.$children) {
									console.warn(
										"Brick added to a container without $children, initializing it",
									);
								...
								const addChildrenToBrickMap = (
									...
									children.forEach((child) => {
										...
										if (child.props?.$children) {
											addChildrenToBrickMap(
												child.props.$children as Brick[],
												child.id,
											);
										}
									});
...
export const useEditorStoreContext = () => {
	...
	invariant(
		store,
		"useEditorStoreContext must be used within a EditorStoreContext",
	);
	return store;
};
...
export const useDraftStoreContext = () => {
	...
	invariant(
		store,
		"useDraftStoreContext must be used within a DraftStoreContext",
	);
	return store;
};
...
export const useGenerationState = () => {
	...
	return useStore(draft, (state) => {
		...
		const isReady =
			hasSitemap && hasThemesGenerated && state.sitemap.length > 0;
		const isGenerating =
			new URL(window.location.href).searchParams.get("action") === "generate";
		return {
			hasSitemap,
			hasThemesGenerated,
...
export function useSelectedSection() {
	...
	return useStore(ctx, (state) => {
		const section = draft
			.getState()
			.sections.find((s) => s.id === state.selectedSectionId);
		if (!section) {
			return null;
		}
		return section;
...
export const useSection = (sectionId?: string) => {
	...
	return useStore(ctx, (state) => {
		...
		return {
			section,
			bricks: Array.from(state.brickMap).filter(
				([, rec]) => rec.sectionId === sectionId,
			),
		};
	});
};
...
export const useSectionsSubscribe = (
	callback: (sections: DraftState["sections"]) => void,
) => {
	const ctx = useDraftStoreContext();
...
export const useAttributesSubscribe = (
	callback: (attr: DraftState["attr"]) => void,
) => {
	const ctx = useDraftStoreContext();
	// biome-ignore lint/correctness/useExhaustiveDependencies: <explanation>
	useEffect(() => {
...
export const useThemeSubscribe = (
	callback: (theme: DraftState["theme"]) => void,
) => {
	const ctx = useDraftStoreContext();
	// biome-ignore lint/correctness/useExhaustiveDependencies: <explanation>
	useEffect(() => {
...
export const usePagePathSubscribe = (
	callback: (path: DraftState["path"]) => void,
) => {
	const ctx = useDraftStoreContext();
	// biome-ignore lint/correctness/useExhaustiveDependencies: <explanation>
	useEffect(() => {
