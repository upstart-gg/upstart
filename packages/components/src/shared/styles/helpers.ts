...
import type {
	FixedPositionedSettings,
	PositionSettings,
} from "@upstart.gg/sdk/shared/bricks/props/position";
...
function getBackgroundColorStyles(
	value: BackgroundColorSettings,
	mobileValue?: BackgroundColorSettings,
) {
	if (!mobileValue) {
...
function getBorderStyles(props?: Partial<BorderSettings>) {
	...
	return [
		propToStyle(color, "borderColor"),
		style,
		borderProcessedClass,
		rounding,
	];
}
...
function getContainerLayoutStyles(
	props?: ContainerLayoutSettings,
	mobileProps?: ContainerLayoutSettings,
) {
...
function getGridStyles(
	...
	return [
		props.type,
		props.columns ? `grid-cols-${props.columns}` : "auto-cols-fr grid-flow-col",
	];
}
