...
export const brickSchema = Type.Object(
	{
		...
		props: Type.Any({
			title: "Props",
			description:
				"The props of the brick. The available props depends on the brick type.",
		}),
		// props: Type.Object({
		//   ...commonProps,
...
export const sectionProps = Type.Object(
	{
		...
		lastTouched: Type.Optional(
			Type.Number({
				description:
					"Do not use this field. It is used internally by the editor.",
				"ui:field": "hidden",
			}),
		),
	},
...
export const sectionSchema = Type.Object(
	{
		id: Type.String({
			description:
				"The unique ID of the section. Use a human readable url-safe slug",
		}),
		label: Type.Optional(
			Type.String({
				description:
					"The label (name) of the section. Used for editor purposes only.",
			}),
		),
		order: Type.Number({
			description:
				"Determines section order in the page (lower numbers appear first). 0-based",
		}),
		props: sectionProps,
		mobileProps: Type.Optional(
			Type.Partial(sectionProps, { additionalProperties: false }),
		),
...
export const sectionDefaultprops = getSchemaObjectDefaults(
	sectionSchema.properties.props,
);
	...
	const result = {
		...
		props: merge({}, defProps.props, {
			...
			...(brick.props.$children
				? { $children: (brick.props.$children as T[]).map(processBrick) }
				: {}),
		}),
	};
	return result;
